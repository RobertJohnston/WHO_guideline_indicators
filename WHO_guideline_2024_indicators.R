...
# Calculation of prevalences of all screening criteria for
...
df <- read_csv(file.path(workdir, "Burkina_Faso-2022-SMART-ANT.csv"))
# View(df)
...
# Plot: Histogram as % + Gaussian density curve
muac_min <- floor(min(df_clean$muac, na.rm = TRUE))
muac_max <- ceiling(max(df_clean$muac, na.rm = TRUE))

# Plot with dynamic x-axis
ggplot(df_clean, aes(x = muac)) +
  geom_histogram(aes(y = ..density.. * 100), 
                 binwidth = 0.2, fill = "red", color = "blue") +
  facet_wrap(~ Region) +
  scale_y_continuous(name = "Percent Density") +
  scale_x_continuous(
    name = "MUAC",
